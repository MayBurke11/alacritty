...
```bash
curl -fsSL https://github.com/MayBurke11/alacritty/releases/download/v0.19.0/alacritty -o ~/.local/bin/alacritty && chmod +x ~/.local/bin/alacritty && mkdir -p ~/.local/share/applications ~/.config/alacritty && cat > ~/.local/share/applications/alacritty.desktop << 'EOF'
[Desktop Entry]
Type=Application
