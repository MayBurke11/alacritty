...
| **Нет управления после создания** | Нельзя закрыть конкретное окно через IPC или переместить существующее. Позиция и размер задаются при создании через `-o`. Закрыть можно через биндинг `action = "Quit"` |
...
- `close-window` — закрыть конкретное окно по ID (сейчас только `Quit` через клавиатуру)
- `send-keys` — послать клавиши в окно
- `read-output` — прочитать содержимое терминала (через PTY)
- `set-position` — изменить позицию существующего окна (при создании позиция задаётся через `-o`)
