...
'
        ...
        alacritty__help__msg)
            opts=\"create-window config get-config create-tab list-tabs select-tab close-tab\"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W \"${opts}\" -- \"${cur}\") )
                return 0
            fi
...
'
            ...
            ;;
        alacritty__msg)
            opts=\"-s -h --socket --help create-window config get-config create-tab list-tabs select-tab close-tab help\"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W \"${opts}\" -- \"${cur}\") )
                return 0
            fi
...
'
        ...
        alacritty__msg__create__tab)
            opts=\"-e -w -h --command --working-directory --no-switch --window-id --help\"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W \"${opts}\" -- \"${cur}\") )
                return 0
            fi
...
'
        ...
        alacritty__msg__help)
            opts=\"create-window config get-config create-tab list-tabs select-tab close-tab help\"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W \"${opts}\" -- \"${cur}\") )
                return 0
            fi
